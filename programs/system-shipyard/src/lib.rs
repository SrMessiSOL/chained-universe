...
declare_id!("74wxuTRib19TzJyXNaeyPVcsFFFqBq8phtRSSPDsK2q2");
...
fn enforce_research_gate(ship_type: u8, r: &Research) -> Result<()> {
    match ship_type {
        0 => require!(r.combustion_drive >= 2, ShipyardError::TechLocked),
        1 => require!(r.combustion_drive >= 6, ShipyardError::TechLocked),
        12 => require!(r.impulse_drive >= 3 && r.astrophysics >= 4, ShipyardError::TechLocked),
        _ => {}
    }
    Ok(())
}
...
pub mod system_shipyard {
    pub fn execute(ctx: Context<Components>, args: Vec<u8>) -> Result<Components> {
        ...
        enforce_research_gate(ship_type, &ctx.accounts.research)?;
