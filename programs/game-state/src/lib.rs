...
declare_id!("HxzRNQiVd9ZcDVvXdcPmCuDjEuqtHCDa4CHgUw7stYm");
...
fn enforce_ship_research_gate(ship_type: u8, planet: &PlanetState) -> Result<()> {
    match ship_type {
        ...
        12 => require!(
            planet.impulse_drive >= 3 && planet.astrophysics >= 4,
...
fn store_cap(level: u8) -> u64 {
    if level == 0 {
        ...
        1_000_000 * 2u64.pow(level as u32)
    }
}
...
fn require_active_session(
    ...
    require_keys_eq!(
        *session_token_info.owner,
        SESSION_PROGRAM_ID,
        GameStateError::InvalidSession
    );
...
pub mod game_state {
    ...
    pub fn initialize_colony(
        ...
        let planet_params = InitializePlanetParams {
            ...
            position: params.position,
            diameter: 8_000u32
                + ((params.galaxy as u32 * 997 + params.system as u32 * 37 + params.position as u32 * 101)
                    % 10_000),
    ...
    pub fn start_build_session(
        ctx: Context<MutatePlanetStateSession>,
        building_idx: u8,
        now: i64,
    ) -> Result<()> {
        require_active_session(
    ...
    pub fn resolve_transport_session(
        ctx: Context<ResolveTransportSession>,
        slot: u8,
        now: i64,
    ) -> Result<()> {
        require_active_session(
    ...
    pub fn resolve_colonize_session(
        ctx: Context<MutatePlanetStateSession>,
        slot: u8,
        now: i64,
    ) -> Result<()> {
    ...
    pub fn commit_planet_state(ctx: Context<CommitPlanetStates>) -> Result<()> {
        let planet_state = ctx.accounts.primary_planet.to_account_info();
        Ok(commit_accounts(
    ...
    pub fn commit_two_planet_states(
        ctx: Context<CommitPlanetStates>,
    ) -> Result<()> {
        let source_planet = ctx.accounts.primary_planet.to_account_info();
        let destination_planet = ctx.accounts.secondary_planet.to_account_info();
        Ok(commit_accounts(
            &ctx.accounts.payer.to_account_info(),
            vec![&source_planet, &destination_planet],
            &ctx.accounts.magic_context.to_account_info(),
            &ctx.accounts.magic_program.to_account_info(),
        )?)
    }

    pub fn commit_and_undelegate_planet_state(
        ctx: Context<CommitPlanetStates>,
    ) -> Result<()> {
        let planet_state = ctx.accounts.primary_planet.to_account_info();
    ...
    pub fn commit_and_undelegate_two_planet_states(
        ctx: Context<CommitPlanetStates>,
    ) -> Result<()> {
        let source_planet = ctx.accounts.primary_planet.to_account_info();
        let destination_planet = ctx.accounts.secondary_planet.to_account_info();
...
pub struct InitializePlanet<'info> {
    ...
    #[account(
        ...
        seeds = [
            ...
            &player_profile.planet_count.to_le_bytes(),
        ],
        bump
    )]
...
pub struct MutatePlanetStateSession<'info> {
    ...
    /// CHECK: Verified against the GPL session program and deserialized
    /// manually so this program can accept a foreign-owned session account.
...
pub struct DelegatePlanetState<'info> {
    ...
    /// CHECK: Verified by deserializing `PlanetState` and recomputing the PDA
    /// seeds before delegating.
    #[account(mut, del)]
...
pub struct CommitPlanetStates<'info> {
    ...
    #[account(mut)]
    pub secondary_planet: Account<'info, PlanetState>,
}
...
pub struct PlanetState {
    ...
    /// Missions remain planet-scoped. Transport can permanently move ships/resources
    /// only when the destination planet has the same authority; otherwise surviving
    /// ships return to the origin planet on the return leg. Colonize burns the
    /// colony ship and moves the carried fleet/resources into the new colony.
...
pub struct MissionState {
    ...
    /// `applied` tracks whether the outward leg effects were already written.
    /// Transport outward resolution delivers to same-owner destinations, while
    /// hostile/foreign destinations trigger a return path. Colonize outward
    /// resolution creates the new colony state and consumes the colony ship.
    pub applied: bool,
}
